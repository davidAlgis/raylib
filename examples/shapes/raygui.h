...
*       layouts must be defined manually on controls drawing, providing the right bounds RectangleRaylib for it.
*
...
*           - void DrawRectangleGradientEx(RectangleRaylib rec, Color col1, Color col2, Color col3, Color col4); // -- GuiColorPicker()
*
...
*           - void SetShapesTexture(Texture2D tex, RectangleRaylib rec);  // -- GuiLoadStyle(), required to set shapes rec to font white rec (optimization)
...
//----------------------------------------------------------------------------------
    ...
    // RectangleRaylib type
    typedef struct RectangleRaylib {
        float x;
        float y;
        float width;
        float height;
    ...
    typedef struct Font {
        ...
        RectangleRaylib *recs;        // Rectangles in texture for the glyphs
...
RAYGUIAPI int GuiWindowBox(RectangleRaylib bounds, const char *title);                                       // Window Box control, shows a window that can be closed
RAYGUIAPI int GuiGroupBox(RectangleRaylib bounds, const char *text);                                         // Group Box control with text name
RAYGUIAPI int GuiLine(RectangleRaylib bounds, const char *text);                                             // Line separator control, could contain text
RAYGUIAPI int GuiPanel(RectangleRaylib bounds, const char *text);                                            // Panel control, useful to group controls
RAYGUIAPI int GuiTabBar(RectangleRaylib bounds, const char **text, int count, int *active);                  // Tab Bar control, returns TAB to be closed or -1
RAYGUIAPI int GuiScrollPanel(RectangleRaylib bounds, const char *text, RectangleRaylib content, Vector2 *scroll, RectangleRaylib *view); // Scroll Panel control

// Basic controls set
RAYGUIAPI int GuiLabel(RectangleRaylib bounds, const char *text);                                            // Label control, shows text
RAYGUIAPI int GuiButton(RectangleRaylib bounds, const char *text);                                           // Button control, returns true when clicked
RAYGUIAPI int GuiLabelButton(RectangleRaylib bounds, const char *text);                                      // Label button control, show true when clicked
RAYGUIAPI int GuiToggle(RectangleRaylib bounds, const char *text, bool *active);                             // Toggle Button control, returns true when active
RAYGUIAPI int GuiToggleGroup(RectangleRaylib bounds, const char *text, int *active);                         // Toggle Group control, returns active toggle index
RAYGUIAPI int GuiToggleSlider(RectangleRaylib bounds, const char *text, int *active);                        // Toggle Slider control, returns true when clicked
RAYGUIAPI int GuiCheckBox(RectangleRaylib bounds, const char *text, bool *checked);                          // Check Box control, returns true when active
RAYGUIAPI int GuiComboBox(RectangleRaylib bounds, const char *text, int *active);                            // Combo Box control, returns selected item index

RAYGUIAPI int GuiDropdownBox(RectangleRaylib bounds, const char *text, int *active, bool editMode);          // Dropdown Box control, returns selected item
RAYGUIAPI int GuiSpinner(RectangleRaylib bounds, const char *text, int *value, int minValue, int maxValue, bool editMode); // Spinner control, returns selected value
RAYGUIAPI int GuiValueBox(RectangleRaylib bounds, const char *text, int *value, int minValue, int maxValue, bool editMode); // Value Box control, updates input text with numbers
RAYGUIAPI int GuiTextBox(RectangleRaylib bounds, char *text, int textSize, bool editMode);                   // Text Box control, updates input text

RAYGUIAPI int GuiSlider(RectangleRaylib bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue); // Slider control, returns selected value
RAYGUIAPI int GuiSliderBar(RectangleRaylib bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue); // Slider Bar control, returns selected value
RAYGUIAPI int GuiProgressBar(RectangleRaylib bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue); // Progress Bar control, shows current progress value
RAYGUIAPI int GuiStatusBar(RectangleRaylib bounds, const char *text);                                        // Status Bar control, shows info text
RAYGUIAPI int GuiDummyRec(RectangleRaylib bounds, const char *text);                                         // Dummy control for placeholders
RAYGUIAPI int GuiGrid(RectangleRaylib bounds, const char *text, float spacing, int subdivs, Vector2 *mouseCell); // Grid control, returns mouse cell position

// Advance controls set
RAYGUIAPI int GuiListView(RectangleRaylib bounds, const char *text, int *scrollIndex, int *active);          // List View control, returns selected list item index
RAYGUIAPI int GuiListViewEx(RectangleRaylib bounds, const char **text, int count, int *scrollIndex, int *active, int *focus); // List View with extended parameters
RAYGUIAPI int GuiMessageBox(RectangleRaylib bounds, const char *title, const char *message, const char *buttons); // Message Box control, displays a message
RAYGUIAPI int GuiTextInputBox(RectangleRaylib bounds, const char *title, const char *message, const char *buttons, char *text, int textMaxSize, bool *secretViewActive); // Text Input Box control, ask for text, supports secret
RAYGUIAPI int GuiColorPicker(RectangleRaylib bounds, const char *text, Color *color);                        // Color Picker control (multiple color controls)
RAYGUIAPI int GuiColorPanel(RectangleRaylib bounds, const char *text, Color *color);                         // Color Panel control
RAYGUIAPI int GuiColorBarAlpha(RectangleRaylib bounds, const char *text, float *alpha);                      // Color Bar Alpha control
RAYGUIAPI int GuiColorBarHue(RectangleRaylib bounds, const char *text, float *value);                        // Color Bar Hue control
RAYGUIAPI int GuiColorPickerHSV(RectangleRaylib bounds, const char *text, Vector3 *colorHsv);                // Color Picker control that avoids conversion to RGB on each call (multiple color controls)
RAYGUIAPI int GuiColorPanelHSV(RectangleRaylib bounds, const char *text, Vector3 *colorHsv);                 // Color Panel control that returns HSV color value, used by GuiColorPickerHSV()
//----------------------------------------------------------------------------------------------------------
...
static RectangleRaylib guiSliderActive = { 0 };       // Gui slider active bounds rectangle, used as an unique identifier
...
static void DrawRectangleGradientEx(RectangleRaylib rec, Color col1, Color col2, Color col3, Color col4); // -- GuiColorPicker()
//-------------------------------------------------------------------------------
...
static void SetShapesTexture(Texture2D tex, RectangleRaylib rec);  // -- GuiLoadStyle(), required to set shapes rec to font white rec (optimization)
...
static bool CheckCollisionPointRec(Vector2 point, RectangleRaylib rec);   // Check if point is inside rectangle
...
static RectangleRaylib GetTextBounds(int control, RectangleRaylib bounds);  // Get text bounds considering control bounds
static const char *GetTextIcon(const char *text, int *iconId);  // Get text icon if provided and move text cursor

static void GuiDrawText(const char *text, RectangleRaylib textBounds, int alignment, Color tint);     // Gui draw text using default font
static void GuiDrawRectangle(RectangleRaylib rec, int borderWidth, Color borderColor, Color color);   // Gui draw rectangle using default raygui style
...
static int GuiScrollBar(RectangleRaylib bounds, int value, int minValue, int maxValue);   // Scroll bar control, used by GuiScrollPanel()
static void GuiTooltip(RectangleRaylib controlRec);                   // Draw tooltip using control rec position
...
// Window Box control
int GuiWindowBox(RectangleRaylib bounds, const char *title)
{
    ...
    RectangleRaylib statusBar = { bounds.x, bounds.y, bounds.width, (float)statusBarHeight };
    if (bounds.height < statusBarHeight*2.0f) bounds.height = statusBarHeight*2.0f;

    RectangleRaylib windowPanel = { bounds.x, bounds.y + (float)statusBarHeight - 1, bounds.width, bounds.height - (float)statusBarHeight + 1 };
    RectangleRaylib closeButtonRec = { statusBar.x + statusBar.width - GuiGetStyle(STATUSBAR, BORDER_WIDTH) - 20,
...
int GuiGroupBox(RectangleRaylib bounds, const char *text)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height }, 0, BLANK, GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));
    GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x, bounds.y + bounds.height - 1, bounds.width, RAYGUI_GROUPBOX_LINE_THICK }, 0, BLANK, GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));
    GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x + bounds.width - 1, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height }, 0, BLANK, GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));

    GuiLine(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x, bounds.y - GuiGetStyle(DEFAULT, TEXT_SIZE)/2, bounds.width, (float)GuiGetStyle(DEFAULT, TEXT_SIZE) }, text);
    //--------------------------------------------------------------------
...
// Line control
int GuiLine(RectangleRaylib bounds, const char *text)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    if (text == NULL) GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x, bounds.y + bounds.height/2, bounds.width, 1 }, 0, BLANK, color);
    else
    {
        RectangleRaylib textBounds = { 0 };
        ...
        GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x, bounds.y + bounds.height/2, RAYGUI_LINE_MARGIN_TEXT - RAYGUI_LINE_TEXT_PADDING, 1 }, 0, BLANK, color);
        GuiDrawText(text, textBounds, TEXT_ALIGN_LEFT, color);
        GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x + 12 + textBounds.width + 4, bounds.y + bounds.height/2, bounds.width - textBounds.width - RAYGUI_LINE_MARGIN_TEXT - RAYGUI_LINE_TEXT_PADDING, 1 }, 0, BLANK, color);
    }
    //--------------------------------------------------------------------
...
// Panel control
int GuiPanel(RectangleRaylib bounds, const char *text)
{
    ...
    RectangleRaylib statusBar = { bounds.x, bounds.y, bounds.width, (float)RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT };
...
int GuiTabBar(RectangleRaylib bounds, const char **text, int count, int *active)
{
    ...
    RectangleRaylib tabBounds = { bounds.x, bounds.y, RAYGUI_TABBAR_ITEM_WIDTH, bounds.height };
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (tabBounds.x < GetScreenWidth())
        {
            ...
            GuiSetStyle(BUTTON, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
#if defined(RAYGUI_NO_ICONS)
            if (GuiButton(RAYGUI_CLITERAL(RectangleRaylib){ tabBounds.x + tabBounds.width - 14 - 5, tabBounds.y + 5, 14, 14 }, "x")) result = i;
#else
            if (GuiButton(RAYGUI_CLITERAL(RectangleRaylib){ tabBounds.x + tabBounds.width - 14 - 5, tabBounds.y + 5, 14, 14 }, GuiIconText(ICON_CROSS_SMALL, NULL))) result = i;
#endif
            GuiSetStyle(BUTTON, BORDER_WIDTH, tempBorderWidth);
            GuiSetStyle(BUTTON, TEXT_ALIGNMENT, tempTextAlignment);
        }
    ...
    GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x, bounds.y + bounds.height - 1, bounds.width, 1 }, 0, BLANK, GetColor(GuiGetStyle(TOGGLE, BORDER_COLOR_NORMAL)));
    //--------------------------------------------------------------------
...
int GuiScrollPanel(RectangleRaylib bounds, const char *text, RectangleRaylib content, Vector2 *scroll, RectangleRaylib *view)
{
    ...
    RectangleRaylib horizontalScrollBar = {
    ...
    RectangleRaylib verticalScrollBar = {
    ...
    *view = (GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)?
                RAYGUI_CLITERAL(RectangleRaylib){ bounds.x + verticalScrollBarWidth + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.width - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth, bounds.height - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth } :
                RAYGUI_CLITERAL(RectangleRaylib){ bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.width - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth, bounds.height - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth };
    ...
    {
        RectangleRaylib corner = { (GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)? (bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH) + 2) : (horizontalScrollBar.x + horizontalScrollBar.width + 2), verticalScrollBar.y + verticalScrollBar.height + 2, (float)horizontalScrollBarWidth - 4, (float)verticalScrollBarWidth - 4 };
...
// Label control
int GuiLabel(RectangleRaylib bounds, const char *text)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiButton(RectangleRaylib bounds, const char *text)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiLabelButton(RectangleRaylib bounds, const char *text)
{
    GuiState state = guiState;
    bool pressed = false;
...
int GuiToggle(RectangleRaylib bounds, const char *text, bool *active)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiToggleGroup(RectangleRaylib bounds, const char *text, int *active)
{
...
int GuiToggleSlider(RectangleRaylib bounds, const char *text, int *active)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiCheckBox(RectangleRaylib bounds, const char *text, bool *checked)
{
    int result = 0;
    GuiState state = guiState;
    ...
    if (*checked)
    {
        RectangleRaylib check = { bounds.x + GuiGetStyle(CHECKBOX, BORDER_WIDTH) + GuiGetStyle(CHECKBOX, CHECK_PADDING),
...
int GuiComboBox(RectangleRaylib bounds, const char *text, int *active)
{
    int result = 0;
    GuiState state = guiState;
    ...
    RectangleRaylib selector = { (float)bounds.x + bounds.width + GuiGetStyle(COMBOBOX, COMBO_BUTTON_SPACING),
...
int GuiDropdownBox(RectangleRaylib bounds, const char *text, int *active, bool editMode)
{
    int result = 0;
    GuiState state = guiState;
...
#if defined(RAYGUI_NO_ICONS)
    GuiDrawText("v", RAYGUI_CLITERAL(RectangleRaylib){ bounds.x + bounds.width - GuiGetStyle(DROPDOWNBOX, ARROW_PADDING), bounds.y + bounds.height/2 - 2, 10, 10 },
                TEXT_ALIGN_CENTER, GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state*3))));
#else
    GuiDrawText("#120#", RAYGUI_CLITERAL(RectangleRaylib){ bounds.x + bounds.width - GuiGetStyle(DROPDOWNBOX, ARROW_PADDING), bounds.y + bounds.height/2 - 6, 10, 10 },
...
int GuiTextBox(RectangleRaylib bounds, char *text, int bufferSize, bool editMode)
{
    ...
    RectangleRaylib mouseCursor = cursor;
    mouseCursor.x = -1;
    mouseCursor.width = 1;
...
bool GuiTextBoxMulti(RectangleRaylib bounds, char *text, int bufferSize, bool editMode)
{
    bool pressed = false;
...
int GuiSpinner(RectangleRaylib bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    int result = 1;
    GuiState state = guiState;
    ...
    RectangleRaylib spinner = { bounds.x + GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH) + GuiGetStyle(SPINNER, SPIN_BUTTON_SPACING), bounds.y,
                          bounds.width - 2*(GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH) + GuiGetStyle(SPINNER, SPIN_BUTTON_SPACING)), bounds.height };
    RectangleRaylib leftButtonBound = { (float)bounds.x, (float)bounds.y, (float)GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.height };
    RectangleRaylib rightButtonBound = { (float)bounds.x + bounds.width - GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.y, (float)GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.height };
...
int GuiValueBox(RectangleRaylib bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    ...
    if (editMode)
    {
        ...
        RectangleRaylib cursor = { bounds.x + GetTextWidth(textValue)/2 + bounds.width/2 + 1, bounds.y + 2*GuiGetStyle(VALUEBOX, BORDER_WIDTH), 4, bounds.height - 4*GuiGetStyle(VALUEBOX, BORDER_WIDTH) };
...
int GuiSliderPro(RectangleRaylib bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue, int sliderWidth)
{
    int result = 0;
    ...
    RectangleRaylib slider = { bounds.x, bounds.y + GuiGetStyle(SLIDER, BORDER_WIDTH) + GuiGetStyle(SLIDER, SLIDER_PADDING),
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        if (guiSliderDragging) // Keep dragging outside of bounds
        {
            ...
            else
            {
                ...
                guiSliderActive = RAYGUI_CLITERAL(RectangleRaylib){ 0, 0, 0, 0 };
            }
        }
...
int GuiSlider(RectangleRaylib bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue)
{
    return GuiSliderPro(bounds, textLeft, textRight, value, minValue, maxValue, GuiGetStyle(SLIDER, SLIDER_WIDTH));
}

// Slider Bar control extended, returns selected value
int GuiSliderBar(RectangleRaylib bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue)
{
    return GuiSliderPro(bounds, textLeft, textRight, value, minValue, maxValue, 0);
}

// Progress Bar control extended, shows current progress value
int GuiProgressBar(RectangleRaylib bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue)
{
    int result = 0;
    GuiState state = guiState;
    ...
    // Progress bar
    RectangleRaylib progress = { bounds.x + GuiGetStyle(PROGRESSBAR, BORDER_WIDTH),
    ...
    else
    {
        if (*value > minValue)
        {
            ...
            GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x, bounds.y, (int)progress.width + (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
            GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x, bounds.y + 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height - 2 }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
            GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x, bounds.y + bounds.height - 1, (int)progress.width + (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
        }
        else GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x, bounds.y, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));

        if (*value >= maxValue) GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x + progress.width + 1, bounds.y, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
        else
        {
            // Draw borders not yet reached by value
            GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x + (int)progress.width + 1, bounds.y, bounds.width - (int)progress.width - 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
            GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x + (int)progress.width + 1, bounds.y + bounds.height - 1, bounds.width - (int)progress.width - 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
            GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ bounds.x + bounds.width - 1, bounds.y + 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height - 2 }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
        }
...
// Status Bar control
int GuiStatusBar(RectangleRaylib bounds, const char *text)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiDummyRec(RectangleRaylib bounds, const char *text)
{
    int result = 0;
    GuiState state = guiState;
...
// List View control
int GuiListView(RectangleRaylib bounds, const char *text, int *scrollIndex, int *active)
{
    int result = 0;
    int itemCount = 0;
...
int GuiListViewEx(RectangleRaylib bounds, const char **text, int count, int *scrollIndex, int *active, int *focus)
{
    int result = 0;
    GuiState state = guiState;
    ...
    if (useScrollBar)
    {
        RectangleRaylib scrollBarBounds = {
...
// Color Panel control
int GuiColorPanel(RectangleRaylib bounds, const char *text, Color *color)
{
    int result = 0;
    GuiState state = guiState;
    ...
    if (state != STATE_DISABLED)
    {
        ...
        RectangleRaylib selector = { pickerSelector.x - GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE)/2, pickerSelector.y - GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE)/2, (float)GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE), (float)GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE) };
...
int GuiColorBarAlpha(RectangleRaylib bounds, const char *text, float *alpha)
{
    ...
    int result = 0;
    GuiState state = guiState;
    RectangleRaylib selector = { (float)bounds.x + (*alpha)*bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT)/2, (float)bounds.y - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT), (float)bounds.height + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)*2 };
    ...
    if (state != STATE_DISABLED)
    {
        ...
        for (int x = 0; x < checksX; x++)
        {
            ...
            {
                RectangleRaylib check = { bounds.x + x*RAYGUI_COLORBARALPHA_CHECKED_SIZE, bounds.y + y*RAYGUI_COLORBARALPHA_CHECKED_SIZE, RAYGUI_COLORBARALPHA_CHECKED_SIZE, RAYGUI_COLORBARALPHA_CHECKED_SIZE };
...
int GuiColorBarHue(RectangleRaylib bounds, const char *text, float *hue)
{
    int result = 0;
    GuiState state = guiState;
    RectangleRaylib selector = { (float)bounds.x - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)bounds.y + (*hue)/360.0f*bounds.height - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT)/2, (float)bounds.width + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)*2, (float)GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT) };
...
//      Color GuiColorPanel(RectangleRaylib bounds, Color color)
//      float GuiColorBarAlpha(RectangleRaylib bounds, float alpha)
//      float GuiColorBarHue(RectangleRaylib bounds, float value)
// NOTE: bounds define GuiColorPanel() size
int GuiColorPicker(RectangleRaylib bounds, const char *text, Color *color)
{
    int result = 0;
    ...
    RectangleRaylib boundsHue = { (float)bounds.x + bounds.width + GuiGetStyle(COLORPICKER, HUEBAR_PADDING), (float)bounds.y, (float)GuiGetStyle(COLORPICKER, HUEBAR_WIDTH), (float)bounds.height };
    //RectangleRaylib boundsAlpha = { bounds.x, bounds.y + bounds.height + GuiGetStyle(COLORPICKER, BARS_PADDING), bounds.width, GuiGetStyle(COLORPICKER, BARS_THICK) };
...
// NOTE: It's divided in multiple controls:
//      int GuiColorPanelHSV(RectangleRaylib bounds, const char *text, Vector3 *colorHsv)
//      int GuiColorBarAlpha(RectangleRaylib bounds, const char *text, float *alpha)
//      float GuiColorBarHue(RectangleRaylib bounds, float value)
// NOTE: bounds define GuiColorPanelHSV() size
int GuiColorPickerHSV(RectangleRaylib bounds, const char *text, Vector3 *colorHsv)
{
    int result = 0;
    ...
    const RectangleRaylib boundsHue = { (float)bounds.x + bounds.width + GuiGetStyle(COLORPICKER, HUEBAR_PADDING), (float)bounds.y, (float)GuiGetStyle(COLORPICKER, HUEBAR_WIDTH), (float)bounds.height };
...
// Message Box control
int GuiMessageBox(RectangleRaylib bounds, const char *title, const char *message, const char *buttons)
{
    ...
    RectangleRaylib buttonBounds = { 0 };
...
int GuiTextInputBox(RectangleRaylib bounds, const char *title, const char *message, const char *buttons, char *text, int textMaxSize, bool *secretViewActive)
{
    ...
    RectangleRaylib textBoxBounds = { 0 };
    ...
    {
        static char stars[] = "****************";
        if (GuiTextBox(RAYGUI_CLITERAL(RectangleRaylib){ textBoxBounds.x, textBoxBounds.y, textBoxBounds.width - 4 - RAYGUI_TEXTINPUTBOX_HEIGHT, textBoxBounds.height },
            ((*secretViewActive == 1) || textEditMode)? text : stars, textMaxSize, textEditMode)) textEditMode = !textEditMode;

        GuiToggle(RAYGUI_CLITERAL(RectangleRaylib){ textBoxBounds.x + textBoxBounds.width - RAYGUI_TEXTINPUTBOX_HEIGHT, textBoxBounds.y, RAYGUI_TEXTINPUTBOX_HEIGHT, RAYGUI_TEXTINPUTBOX_HEIGHT }, (*secretViewActive == 1)? "#44#" : "#45#", secretViewActive);
    }
    else
    {
...
int GuiGrid(RectangleRaylib bounds, const char *text, float spacing, int subdivs, Vector2 *mouseCell)
{
    ...
    if (subdivs > 0)
    {
        ...
        {
            RectangleRaylib lineV = { bounds.x + spacing*i/subdivs, bounds.y, 1, bounds.height };
        ...
        {
            RectangleRaylib lineH = { bounds.x, bounds.y + spacing*i/subdivs, bounds.width, 1 };
...
void GuiLoadStyleDefault(void)
{
    ...
    if (guiFont.texture.id != GetFontDefault().texture.id)
    {
        ...
        RectangleRaylib whiteChar = guiFont.recs[95];

        // NOTE: We set up a 1px padding on char rectangle to avoid pixel bleeding on MSAA filtering
        SetShapesTexture(guiFont.texture, RAYGUI_CLITERAL(RectangleRaylib){ whiteChar.x + 1, whiteChar.y + 1, whiteChar.width - 2, whiteChar.height - 2 });
    }
}
...
void GuiDrawIcon(int iconId, int posX, int posY, int pixelSize, Color color)
{
    ...
    for (int i = 0, y = 0; i < RAYGUI_ICON_SIZE*RAYGUI_ICON_SIZE/32; i++)
    {
        for (int k = 0; k < 32; k++)
        {
            ...
            {
            #if !defined(RAYGUI_STANDALONE)
                GuiDrawRectangle(RAYGUI_CLITERAL(RectangleRaylib){ (float)posX + (k%RAYGUI_ICON_SIZE)*pixelSize, (float)posY + y*pixelSize, (float)pixelSize, (float)pixelSize }, 0, BLANK, color);
            #endif
            }
...
static void GuiLoadStyleFromMemory(const unsigned char *fileData, int dataSize)
{
        ...
        (signature[3] == ' '))
    {
        ...
        if (fontDataSize > 0)
        {
            ...
            RectangleRaylib fontWhiteRec = { 0 };
            memcpy(&fontWhiteRec, fileDataPtr, sizeof(RectangleRaylib));
            fileDataPtr += 16;
            ...
            {
                // Load font recs data
                int recsDataSize = font.glyphCount*sizeof(RectangleRaylib);
                ...
                if ((recsDataCompressedSize > 0) && (recsDataCompressedSize != recsDataSize))
                {
                    ...
                    font.recs = (RectangleRaylib *)DecompressData(recsDataCompressed, recsDataCompressedSize, &recsDataUncompSize);
                ...
                else
                {
                    ...
                    font.recs = (RectangleRaylib *)RAYGUI_CALLOC(font.glyphCount, sizeof(RectangleRaylib));
                    for (int i = 0; i < font.glyphCount; i++)
                    {
                        memcpy(&font.recs[i], fileDataPtr, sizeof(RectangleRaylib));
                        fileDataPtr += sizeof(RectangleRaylib);
                    }
                }
...
static RectangleRaylib GetTextBounds(int control, RectangleRaylib bounds)
{
    RectangleRaylib textBounds = bounds;
...
static void GuiDrawText(const char *text, RectangleRaylib textBounds, int alignment, Color tint)
{
...
static void GuiDrawRectangle(RectangleRaylib rec, int borderWidth, Color borderColor, Color color)
{
    if (color.a > 0)
    {
...
static void GuiTooltip(RectangleRaylib controlRec)
{
    if (!guiLocked && guiTooltip && (guiTooltipPtr != NULL) && !guiSliderDragging)
    {
        ...
        GuiPanel(RAYGUI_CLITERAL(RectangleRaylib){ controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16, GuiGetStyle(DEFAULT, TEXT_SIZE) + 8.f }, NULL);

        int textPadding = GuiGetStyle(LABEL, TEXT_PADDING);
        int textAlignment = GuiGetStyle(LABEL, TEXT_ALIGNMENT);
        GuiSetStyle(LABEL, TEXT_PADDING, 0);
        GuiSetStyle(LABEL, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
        GuiLabel(RAYGUI_CLITERAL(RectangleRaylib){ controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16, GuiGetStyle(DEFAULT, TEXT_SIZE) + 8.f }, guiTooltipPtr);
...
static int GuiScrollBar(RectangleRaylib bounds, int value, int minValue, int maxValue)
{
    GuiState state = guiState;
    ...
    // Arrow buttons [<] [>] [∧] [∨]
    RectangleRaylib arrowUpLeft = { 0 };
    RectangleRaylib arrowDownRight = { 0 };

    // Actual area of the scrollbar excluding the arrow buttons
    RectangleRaylib scrollbar = { 0 };

    // Slider bar that moves     --[///]-----
    RectangleRaylib slider = { 0 };
    ...
    arrowUpLeft = RAYGUI_CLITERAL(RectangleRaylib){
    ...
    if (isVertical)
    {
        arrowDownRight = RAYGUI_CLITERAL(RectangleRaylib){ (float)bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)bounds.y + bounds.height - spinnerSize - GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)spinnerSize, (float)spinnerSize };
        scrollbar = RAYGUI_CLITERAL(RectangleRaylib){ bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING), arrowUpLeft.y + arrowUpLeft.height, bounds.width - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING)), bounds.height - arrowUpLeft.height - arrowDownRight.height - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH) };
        ...
        slider = RAYGUI_CLITERAL(RectangleRaylib){
    ...
    {
        arrowDownRight = RAYGUI_CLITERAL(RectangleRaylib){ (float)bounds.x + bounds.width - spinnerSize - GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)spinnerSize, (float)spinnerSize };
    ...
    if (GuiGetStyle(SCROLLBAR, ARROWS_VISIBLE))
    {
        ...
        GuiDrawText(isVertical? "^" : "<",
            RAYGUI_CLITERAL(RectangleRaylib){ arrowUpLeft.x, arrowUpLeft.y, isVertical? bounds.width : bounds.height, isVertical? bounds.width : bounds.height },
            TEXT_ALIGN_CENTER, GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state*3))));
        GuiDrawText(isVertical? "v" : ">",
            RAYGUI_CLITERAL(RectangleRaylib){ arrowDownRight.x, arrowDownRight.y, isVertical? bounds.width : bounds.height, isVertical? bounds.width : bounds.height },
            TEXT_ALIGN_CENTER, GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state*3))));
#else
...
static bool CheckCollisionPointRec(Vector2 point, RectangleRaylib rec)
{
...
{
    RectangleRaylib bounds = { (float)posX, (float)posY, (float)width, (float)height };
