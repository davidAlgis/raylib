...
typedef struct LightInfo {
    ...
    RectangleRaylib bounds;           // A cached rectangle of the light bounds to help with culling
...
bool UpdateLight(int slot, RectangleRaylib* boxes, int count)
{
...
// Set up some boxes
void SetupBoxes(RectangleRaylib *boxes, int *count)
{
    boxes[0] = (RectangleRaylib){ 150,80, 40, 40 };
    boxes[1] = (RectangleRaylib){ 1200, 700, 40, 40 };
    boxes[2] = (RectangleRaylib){ 200, 600, 40, 40 };
    boxes[3] = (RectangleRaylib){ 1000, 50, 40, 40 };
    boxes[4] = (RectangleRaylib){ 500, 350, 40, 40 };

    for (int i = 5; i < MAX_BOXES; i++)
    {
        boxes[i] = (RectangleRaylib){(float)GetRandomValue(0,GetScreenWidth()), (float)GetRandomValue(0,GetScreenHeight()), (float)GetRandomValue(10,100), (float)GetRandomValue(10,100) };
    }
...
int main(void)
{
    ...
    int boxCount = 0;
    RectangleRaylib boxes[MAX_BOXES] = { 0 };
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        if (dirtyLights)
        {
            ...
            BeginTextureMode(lightMask);
                ...
                for (int i = 0; i < MAX_LIGHTS; i++)
                {
                    if (lights[i].active) DrawTextureRec(lights[i].mask.texture, (RectangleRaylib){ 0, 0, (float)GetScreenWidth(), -(float)GetScreenHeight() }, Vector2Zero(), WHITE);
                }
        ...
        BeginDrawing();
            ...
            DrawTextureRec(backgroundTexture, (RectangleRaylib){ 0, 0, (float)GetScreenWidth(), (float)GetScreenHeight() }, Vector2Zero(), WHITE);
            
            // Overlay the shadows from all the lights
            DrawTextureRec(lightMask.texture, (RectangleRaylib){ 0, 0, (float)GetScreenWidth(), -(float)GetScreenHeight() }, Vector2Zero(), ColorAlpha(WHITE, showLines? 0.75f : 1.0f));
