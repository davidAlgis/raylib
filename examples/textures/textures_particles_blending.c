    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        BeginDrawing();
            ...
            BeginBlendMode(blending);
                ...
                for (int i = 0; i < MAX_PARTICLES; i++)
                {
                    if (mouseTail[i].active) DrawTexturePro(smoke, (RectangleRaylib){ 0.0f, 0.0f, (float)smoke.width, (float)smoke.height },
                                                           (RectangleRaylib){ mouseTail[i].position.x, mouseTail[i].position.y, smoke.width*mouseTail[i].size, smoke.height*mouseTail[i].size },
