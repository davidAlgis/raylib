...
Image ImageFromImage(Image image, RectangleRaylib rec)
{
    Image result = { 0 };
...
void ImageCrop(Image *image, RectangleRaylib crop)
{
...
Image ImageTextEx(Font font, const char *text, float fontSize, float spacing, Color tint)
{
    ...
    for (int i = 0; i < size;)
    {
        ...
        else
        {
            ...
            {
                RectangleRaylib rec = { (float)(textOffsetX + font.glyphs[index].offsetX), (float)(textOffsetY + font.glyphs[index].offsetY), (float)font.recs[index].width, (float)font.recs[index].height };
                ImageDraw(&imText, font.glyphs[index].image, (RectangleRaylib){ 0, 0, (float)font.glyphs[index].image.width, (float)font.glyphs[index].image.height }, rec, tint);
            }
...
void ImageResizeCanvas(Image *image, int newWidth, int newHeight, int offsetX, int offsetY, Color fill)
{
    ...
    {
        RectangleRaylib srcRec = { 0, 0, (float)image->width, (float)image->height };
...
void ImageAlphaCrop(Image *image, float threshold)
{
    ...
    RectangleRaylib crop = GetImageAlphaBorder(*image, threshold);
...
// NOTE: Threshold is defined as a percentage: 0.0f -> 1.0f
RectangleRaylib GetImageAlphaBorder(Image image, float threshold)
{
    RectangleRaylib crop = { 0 };
    ...
    if (pixels != NULL)
    {
        ...
        {
            crop = (RectangleRaylib){ (float)xMin, (float)yMin, (float)((xMax + 1) - xMin), (float)((yMax + 1) - yMin) };
        }
...
{
    ImageDrawRectangleRec(dst, (RectangleRaylib){ (float)posX, (float)posY, (float)width, (float)height }, color);
}
...
// Draw rectangle within an image
void ImageDrawRectangleRec(Image *dst, RectangleRaylib rec, Color color)
{
    // Security check to avoid program crash
    if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0)) return;
...
void ImageDrawRectangleLines(Image *dst, RectangleRaylib rec, int thick, Color color)
{
...
void ImageDraw(Image *dst, Image src, RectangleRaylib srcRec, RectangleRaylib dstRec, Color tint)
{
    // Security check to avoid program crash
    ...
    else
    {
        ...
        if (((int)srcRec.width != (int)dstRec.width) || ((int)srcRec.height != (int)dstRec.height))
        {
            ...
            srcRec = (RectangleRaylib){ 0, 0, (float)srcMod.width, (float)srcMod.height };
...
void ImageDrawTextEx(Image *dst, Font font, const char *text, Vector2 position, float fontSize, float spacing, Color tint)
{
    ...
    RectangleRaylib srcRec = { 0.0f, 0.0f, (float)imText.width, (float)imText.height };
    RectangleRaylib dstRec = { position.x, position.y, (float)imText.width, (float)imText.height };
...
TextureCubemap LoadTextureCubemap(Image image, int layout)
{
    ...
    if (layout != CUBEMAP_LAYOUT_AUTO_DETECT)
    {
        ...
        RectangleRaylib faceRecs[6] = { 0 };      // Face source rectangles
        for (int i = 0; i < 6; i++) faceRecs[i] = (RectangleRaylib){ 0, 0, (float)size, (float)size };
        ...
        else
        {
            ...
            for (int i = 0; i < 6; i++) ImageDraw(&faces, image, faceRecs[i], (RectangleRaylib){ 0, (float)size*i, (float)size, (float)size }, WHITE);
        }
...
// NOTE: pixels data must match texture.format
void UpdateTextureRec(Texture2D texture, RectangleRaylib rec, const void *pixels)
{
...
{
    RectangleRaylib source = { 0.0f, 0.0f, (float)texture.width, (float)texture.height };
    RectangleRaylib dest = { position.x, position.y, (float)texture.width*scale, (float)texture.height*scale };
...
void DrawTextureRec(Texture2D texture, RectangleRaylib source, Vector2 position, Color tint)
{
    RectangleRaylib dest = { position.x, position.y, fabsf(source.width), fabsf(source.height) };
    Vector2 origin = { 0.0f, 0.0f };
...
void DrawTexturePro(Texture2D texture, RectangleRaylib source, RectangleRaylib dest, Vector2 origin, float rotation, Color tint)
{
...
void DrawTextureNPatch(Texture2D texture, NPatchInfo nPatchInfo, RectangleRaylib dest, Vector2 origin, float rotation, Color tint)
{
    if (texture.id > 0)
    {
        float width = (float)texture.width;
